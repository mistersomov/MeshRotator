...
void scene::MainScene::renderOutline(Model& model) const {
    ...
    glm::mat4 modelMat =
            ...
            * glm::scale(glm::mat4(1.0f), scaled*(model.getScale()));
...
void scene::MainScene::renderNormalVector(scene::Model &model) const {
    glm::mat4 modelMat =
        glm::translate(glm::mat4(1.0f), model.getPosition())
            * glm::rotate(
                glm::mat4(1.0f),
    ...
    normalVectorShader_->set_mat4("MODEL", modelMat);
    model.draw(*normalVectorShader_);
}
...
void scene::MainScene::initShaders() {
    ...
    pillarShader_ = shaderManager.getShader("shader/baseGeom.vert", "shader/baseGeom.frag");
    outlineShader_ =  shaderManager.getShader("shader/baseGeom.vert", "shader/outlined.frag");
    normalVectorShader_ = shaderManager.getShader(
        "shader/normalVisualization.vert",
        "shader/normalVisualization.frag",
        "shader/normalVisualization.geom"
    );
    screenShader_ = shaderManager.getShader("shader/framebufferScreen.vert", "shader/framebufferScreen.frag");
}
...
void scene::MainScene::initUniformBuffers() {
    ...
    unsigned int uniformBlockIndexNormal =
        glGetUniformBlockIndex(normalVectorShader_->id_, "Matrices");
    glUniformBlockBinding(pillarShader_->id_, uniformBlockIndexPillar, 0);
    glUniformBlockBinding(normalVectorShader_->id_, uniformBlockIndexNormal, 0);
...
void scene::MainScene::addDirectionalLights() const {
    pillarShader_->activate();
    pillarShader_->set_vec3("dirLight.direction", glm::vec3{-1.0f, -1.0f, 0.0f});
...
void scene::MainScene::addPillar() {
    ...
    pillar.setPosition({-5.0f, 0.0f, 0.0f});
...
void scene::MainScene::addBrokenPillar() {
    ...
    modelManager.applyTextures(pillar, textures);
    pillar.setPosition({5.0f, -2.0f, 0.0f});
    pillar.setScale(glm::vec3(1.0f));
    models_.emplace_back(pillar);
}
